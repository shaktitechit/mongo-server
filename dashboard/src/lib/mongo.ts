...
export async function createConnectedClient(dbName: string = "admin"): Promise<MongoClient> {
  ...
  for (const target of targets) {
    const uri = `mongodb://${rootUser}:${encodeURIComponent(rootPass)}@${target.host}:${target.port}/${dbName}?authSource=admin&directConnection=true`;
    try {
